...
public class RunTest {
	...
	public static void main(String[] args) {
		...
		/*
		// 1. 각자 pc(localhost)에 JDBC계정에 연결한 후 TEST에이블에 insert 해보기
		...
		// 앞으로 실행할 sql문("완성형태"로 만들어 두기)(맨 뒤에 세미클론은 없어야된다!!)
		...
		try {
			...
			// 4,5) sql문 전달 실행 후 결과 받기(처리된 행수)
			result = stmt.executeUpdate(sql);
			// 내가 실행할 sql문이 dml문(insert, update, delete)일 경우 => stmt.excuteUpdate("dml문") :
			// int
			...
			try {
				// 7) 다 사용한 JDBC 객체 자원 반납(생성된 역순으로)
				stmt.close();
				conn.close();
			} catch (SQLException e) {
				e.printStackTrace();
