...
public class MemberController {
	...
	public void selectByUserName(String inputMemberName) {
		...
		ArrayList<Member> list = new MemberDao().selectByUserName(inputMemberName);
		
		if(list.size() > 0) {
			new Membermenu().displayMemberList(list);
		} else {
			new Membermenu().displayNoData(inputMemberName);
	...
	public void deleteMember(String userId) {
		...
		if(result > 0) {
			...
			new Membermenu().displayFail("회원 탈퇴처리에 실패하였습니다.");
		}
